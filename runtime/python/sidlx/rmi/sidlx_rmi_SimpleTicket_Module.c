...
_loadClassImpl(void) {
  if (!_implEPV) {
      ...
      if (dll) {
        ...
        if (!throwaway_exception && _extFunc) {
          ...
          errmsg = sidl_String_concat3(
            "babel: The shared library containing SIDL type sidlx.rmi.SimpleTicket, ",
            name,
            ", was loaded but is missing a required symbol (sidlx_rmi_SimpleTicket__externals).");
          sidl_String_free(name);
        }
        sidl_DLL_deleteRef(dll, &throwaway_exception);
      }
      else {
        errmsg = sidl_String_strdup("babel: Unable to find or load a shared library containing the implementation of SIDL type sidlx.rmi.SimpleTicket. To get more debugging information trying setting the SIDL_DEBUG_DLOPEN environment variable and rerunning.");
      }
    }
#if (PY_VERSION_HEX >= 0x02040000)
    Py_END_ALLOW_THREADS
    ...
    if (!_implEPV) {
      ...
      else PyErr_SetString(PyExc_RuntimeError, "Unexpected error loading SIDL implementation for sidlx.rmi.SimpleTicket.");
...
{
  struct sidlx_rmi_SimpleTicket__object *sidlobj = NULL;
