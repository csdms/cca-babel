...
static PyObject *
pStub_ServerRegistry_getServer(PyObject *_ignored, PyObject *_args, PyObject  \
  *_kwdict) {
  PyObject *_return_value = NULL;
  struct sidl_BaseInterface__object *_exception = NULL;
  static char *_kwlist[] = {
  ...
  _okay = PyArg_ParseTupleAndKeywords(
    ...
    "", _kwlist);
  if (_okay) {
    struct sidl_rmi_ServerInfo__object* _return = NULL;
#if (PY_VERSION_HEX >= 0x02040000)
    Py_BEGIN_ALLOW_THREADS
    sidl_Python_LogUnlock(__func__, __FILE__, __LINE__);
#endif /* Python 2.4 or later */
    _return = (*(_sepv->f_getServer))(&_exception);
#if (PY_VERSION_HEX >= 0x02040000)
    Py_END_ALLOW_THREADS
    sidl_Python_LogRelock(__func__, __FILE__, __LINE__);
#endif /* Python 2.4 or later */
    if (_exception) {
      ...
      _return_value = Py_BuildValue(
        "O&",
        (void *)sidl_rmi_ServerInfo__wrap, _return);
    }
  }
  return _return_value;
}
...
IS SOMETHING OF A TEST! THIS MAY CHANGE!"
  ...
  { "getServer", (PyCFunction)pStub_ServerRegistry_getServer,
  (METH_VARARGS | METH_KEYWORDS),
"\
getServer()\n\
RETURNS\n\
   (sidl.rmi.ServerInfo _return)\n\
RAISES\n\
    sidl.RuntimeException\n\
\n\
\
...
{
  struct sidl_rmi_ServerRegistry__object *sidlobj = NULL;
...
one server is assumed per Babel process)"
  ...
  _sepv = (*_implEPV->getStaticEPV)();
  if (PyErr_Occurred()) {
    Py_FatalError("Cannot initialize Python module sidl.rmi.ServerRegistry.");
  }
