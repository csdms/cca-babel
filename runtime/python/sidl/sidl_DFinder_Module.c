  ...
  sidl_BaseInterface__object* *_ex)
{
  ...
  *_ex = NULL;
  ...
  if(objectID) {
    ...
    retobj = (struct sidl_DFinder__object*) (*bi->d_epv->f__cast)(            \
      bi->d_object, "sidl.DFinder", _ex);
    if(!ar) { 
      (*bi->d_epv->f_deleteRef)(bi->d_object, &throwaway_exception);
    }
    return retobj;
...
{
  struct sidl_DFinder__object *sidlobj = NULL;
...
environment variable."
  ...
  if (PyErr_Occurred()) {
    Py_FatalError("Cannot initialize Python module sidl.DFinder.");
  }
